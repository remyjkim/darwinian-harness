...
- The value-free Worker v2 adapter matches the exact reviewed I268 schema, confines both CLI paths, persists only through a mode-`0600` same-directory temp plus close/sync/atomic rename, emits no success stdout, cleans every pre-commit failure, and cannot become a control-plane/fallback path.
