...
No source or RED test task begins until the corrected architecture passes replacement formal G1 and this corrected plan passes replacement ordered G2. They may share one exact review ref, but the gate record remains G1 then G2. The I268 comparison contract at exact ref `75970eb0` has already passed its own ordered G1/G2; this satisfies only the cross-lane schema dependency and does not authorize I265 source. Both producer-owned application comparators are acknowledged. I265 events `i265-owner-20260809T235736Z-015` and `i265-owner-20260809T235950Z-016`, plus I266 event `i266-owner-reviewer-20260809T235908Z-212`, accept the exact 1,225-byte producer-enforced rule config at SHA-256 `32225d0b5dda0d2a7ad37981d7441cde12a83a1200d2bdafbff25add0f300c2a`. Any later I268 schema, config, path, command, bound, ownership, or rule drift reopens this dependency.

Coordinator events `architecture-coordinator-20260810T004351Z-032`, `architecture-coordinator-20260810T005202Z-034`, and `architecture-coordinator-20260810T033057Z-038` supersede the earlier I265 G1/G2 passes. Task 1 commit `d3562399974c0a29439938f1677c79ca4aab63f9` and Task 2 commit `d69f437c71a47801db99af808a96ffcdcba85bd3` are preserved without destructive rollback as frozen, unaccepted work. Neither may be reused, extended, merged, or represented as gate-authorized until replacement ordered I265 G1/G2 passes and an exact reuse review accepts it.
...
This task is held until replacement I265 G2 passes. Exact I268 ref `75970eb0` has already received ordered I268 G1/G2 PASS; before RED, re-read the tracker/log, reverify its exact digests and the accepted `cl.i268.finch-derivation-input.v1` and `cl.i268.finch-derivation-output.v2` schemas, and append the dependency transition to Notion/the shared log. Any I268 schema/path/command drift reopens the dependency.
...
9. only the `--input` and `--output` CLI operands are path controls; each is a relative normalized path confined beneath the real process-start working directory, with absolute/empty/dot/traversal/NUL/outside-root/symlink paths, missing parents, non-regular input, pre-existing final output, or identical input/output identity rejected before derivation or output creation;
...
13. success serializes and size-checks complete canonical bytes in memory; after output-parent realpath admission it opens the already-existing parent, freezes its handle-derived device/inode, revalidates the canonical pathname before temp open, exclusively creates one unpredictable same-directory temporary regular file relative to that handle at mode `0600` where supported, captures its identity, writes all bytes, syncs and closes it, revalidates parent plus descriptor-relative owned temp immediately before publication, commits with descriptor-bound same-directory `linkat` or an exact atomic no-replace equivalent, revalidates parent plus descriptor-relative final target immediately after link, syncs the opened parent-directory handle, unlinks only the identity-matched owned temporary name relative to that handle, syncs that directory handle again, and revalidates parent plus final before identity/success; path-only link, ordinary rename, copy, direct-final write, and any replace-capable fallback are forbidden; only completion of both directory syncs and all identity checks is clean success, which emits no stdout and leaves no temporary residue;
14. inject a competing destination after admission and the early absence precheck but immediately before `link`; require `EEXIST` to map to a stable sanitized output-exists failure, preserve the competing destination byte-for-byte, clean only the temporary pathname still proven to be the captured owned inode, directory-sync that cleanup, and perform no retry or replace-capable fallback;
15. replace the admitted parent pathname with another directory or symlink (a) after realpath but before temp open and (b) after temp persistence but immediately before link; each must fail closed without writing outside the frozen admitted directory, qualify no output, preserve any foreign path, and clean only a temp independently proven to be owned; then substitute parent or final target immediately after link and prove post-link/pre-success revalidation qualifies nothing, removes no final, and forbids blind retry;
16. inject failure of the first parent-directory sync after successful `link`; require sanitized `WORKER_RUNTIME_ADMISSION_OUTPUT_COMMIT_INDETERMINATE`, both final and temporary names left untouched, no success claim, no final removal/replacement, and no blind retry;
17. after a successful first directory sync, inject owned-temp unlink failure and require sanitized `WORKER_RUNTIME_ADMISSION_OUTPUT_COMMITTED_TEMP_CLEANUP_FAILED`; then separately inject failure of the second directory sync after successful unlink and require sanitized `WORKER_RUNTIME_ADMISSION_OUTPUT_COMMITTED_CLEANUP_DURABILITY_INDETERMINATE`; both preserve the durably committed final, neither reports clean success, and neither permits blind retry;
18. injected parse/identity/derivation/serialization/open/short-write/file-sync/close/link failures emit only a stable sanitized stderr code, remove only a temp pathname still proven to be the captured owned inode, directory-sync any successful pre-commit cleanup unlink, leave no final result before commit, and never truncate/overwrite/remove a pre-existing path; a directory/temp/final identity mismatch deletes no foreign object, and unsupported hard-link, inode-revalidation, or directory-open/sync semantics fail closed as `WORKER_RUNTIME_ADMISSION_OUTPUT_PERSISTENCE_UNSUPPORTED` without a replace-capable fallback;
19. candidate/receipt self-identity is absent while the externally computed serialized-artifact identity remains downstream; and
20. hostile nested manifest command/environment values and lock path/Git URL values cause no process, filesystem, environment, network, output, or diagnostic side effect before rejection or successful pure derivation; forged caller `pass`, changed rule/config digest, URL userinfo/query/fragment, authorization/cookie/API-key header, unexpected env/header/provider, and missing/changed byte coverage fail without output or raw diagnostics.

### GREEN

Implement the minimum offline adapter around `cli/core/runtime-admission-manifest.ts`. Capture the process-start working-directory realpath as the sole confinement root; accept only the two reviewed CLI path operands under it; reject path escape, symlink, alias, existing-output, and non-regular-input cases; and create no directory. Accept only the reviewed bounded passive derivation-input file, reverify its closed byte identities, and perform no ambient lookup or execution. Treat every nested manifest/lock path, command, and URL as inert bytes. Independently run the exact frozen target-specific rule: allow only the reviewed `buzz-tools` server command/args/optionality with provider/env/headers/url absent, no root server, the two exact public Finch Git URLs with empty userinfo/query/fragment, bounded inert lock paths, and closed empty/absent auxiliary fields. The rule's `file|store|git` origin set does not override the production deploy path's separate `file`/`npm` rejection. Bind the producer's own rule pass/config digest in output v2 and never echo nested values. Derive Worker-owned values; validate/bind I268-owned phase/publication inputs without creating them. Open and freeze the existing output parent's device/inode after realpath admission. Use descriptor-relative temp creation, identity checks, no-replace link, and cleanup so a mutable canonical pathname never regains filesystem authority; also require pathname-to-frozen-inode revalidation before temp open, parent plus owned-temp revalidation immediately before link, and parent plus final-target revalidation immediately after link and again before identity/success. If those descriptor-bound operations, identity checks, directory sync, or hard-link semantics are unsupported, fail closed before qualification. Persist the one closed v2 result through exclusive mode-`0600` same-directory temp creation, complete write, file sync, close, atomic no-replace hard-link commit, first directory sync, identity-safe temp unlink, and second directory sync. Treat the early destination check only as a diagnostic. On `EEXIST`, preserve the competing final byte-for-byte and directory-sync removal of only the temp still proven to be the captured owned inode. Never fall back to path-only link, ordinary rename, copy, direct-final write, or another replace-capable operation. Implement the four explicit post-link outcomes from RED: clean success, commit indeterminate, committed temp-cleanup failure, and committed cleanup-durability indeterminate; every non-clean result preserves the final and forbids blind retry. Add the package command without registering a deploy/network CLI control plane.
...
- The value-free Worker v2 adapter matches the exact reviewed I268 schema, confines both CLI paths, freezes/revalidates output-parent and temp/final inode identities across pre-open, pre-link, post-link, and pre-success boundaries, and persists only through a mode-`0600` same-directory temp plus file sync, atomic no-replace hard-link commit, first parent-directory sync, identity-safe temp unlink, and second parent-directory sync; it preserves a destination introduced after precheck byte-for-byte, exposes explicit indeterminate/committed-cleanup outcomes, never reports clean success with temp residue, emits no success stdout, and cannot become a control-plane/fallback path.
...
| Destination appears after precheck | Same-filesystem `link(temp, final)` commit fails `EEXIST`; no rename/copy fallback, competing bytes unchanged, only identity-matched owned temp may be removed |
| Namespace update is not crash durable | Sync the opened parent directory after final link and again after temp unlink; first-sync, unlink, and second-sync failures have distinct non-success outcomes and never remove the committed final |
| Parent path is substituted after admission | Freeze admitted directory device/inode; use descriptor-relative create/link/check/unlink authority; revalidate pathname before temp open, before link with owned temp, after link with owned final, and before success; mismatch qualifies nothing and deletes no foreign object |

## Stop conditions

Stop, append a coordination event, and add a Notion checkpoint if G1/G2 is absent or changed; the adapter task lacks exact I268 G2 PASS; origin changes frozen seams; lock preservation needs a broader break; validation cannot precede effects; the platform/filesystem cannot provide the reviewed atomic no-replace `link`, opened-parent-directory, inode-revalidation, and directory-sync semantics; cleanup cannot prove the temporary pathname is still the adapter-owned captured inode; any implementation remains path-only, can qualify a substituted parent/final, or would report clean success before both directory syncs or with temp residue; caller data becomes derived authority; v1 would widen transport/auth/probes; a test needs network/credentials/publication/migration/deployment/mode/lease/candidate/traffic; or I267/I268 ownership would be crossed.
