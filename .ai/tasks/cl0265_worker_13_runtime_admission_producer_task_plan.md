...
Coordinator events `architecture-coordinator-20260810T004351Z-032`, `architecture-coordinator-20260810T005202Z-034`, and `architecture-coordinator-20260810T033057Z-038` supersede the earlier I265 G1/G2 passes and reject ordinary rename as the final output commit. Task 1 commit `d3562399974c0a29439938f1677c79ca4aab63f9` and Task 2 commit `d69f437c71a47801db99af808a96ffcdcba85bd3` are preserved without destructive rollback as frozen, unaccepted work. Neither may be reused, extended, merged, or represented as gate-authorized until replacement ordered I265 G1/G2 passes and an exact reuse review accepts it. Event `architecture-coordinator-20260810T040210Z-050` supplies the accepted cross-lane no-replace precedent: same-directory hard link, `EEXIST` preservation, owned-inode cleanup, temp unlink, and directory fsync.
...
13. success serializes and size-checks complete canonical bytes in memory, exclusively creates one unpredictable same-directory temporary regular file at mode `0600` where supported, records/verifies its device/inode identity, writes all bytes, syncs and closes it, hard-links that inode to the absent target without replacement, verifies final inode identity, unlinks the temp, fsyncs the directory, emits no stdout, and returns only after the durable namespace commit;
14. a controlled destination created after the last absence observation but immediately before hard-link commit causes atomic `EEXIST`, stable `WORKER_RUNTIME_ADMISSION_ADAPTER_OUTPUT_EXISTS`, unchanged competing bytes, no output/result identity/stdout, and removal of only the exact owned temporary inode;
15. ordinary rename/copy/truncate/overwrite is absent; an unsupported or non-enforceable hard-link primitive fails closed rather than selecting a compatibility fallback;
16. injected parse/identity/derivation/serialization/open/short-write/file-sync/close/hard-link/post-link-identity/unlink/directory-sync failures emit only stable `WORKER_RUNTIME_ADMISSION_ADAPTER_PERSIST_FAILED` stderr, return no result identity, and leave no qualifying target or owned temp; late cleanup removes a target/temp name only when fresh device/inode comparison proves it still identifies the owned inode and never mutates unrelated or competing bytes;
17. candidate/receipt self-identity is absent while the externally computed serialized-artifact identity remains downstream; and
18. hostile nested manifest command/environment values and lock path/Git URL values cause no process, filesystem, environment, network, output, or diagnostic side effect before rejection or successful pure derivation; forged caller `pass`, changed rule/config digest, URL userinfo/query/fragment, authorization/cookie/API-key header, unexpected env/header/provider, and missing/changed byte coverage fail without output or raw diagnostics.

### GREEN

Implement the minimum offline adapter around `cli/core/runtime-admission-manifest.ts`. Capture the process-start working-directory realpath as the sole confinement root; accept only the two reviewed CLI path operands under it; reject path escape, symlink, alias, existing-output, and non-regular-input cases; and create no directory. Accept only the reviewed bounded passive derivation-input file, reverify its closed byte identities, and perform no ambient lookup or execution. Treat every nested manifest/lock path, command, and URL as inert bytes. Independently run the exact frozen target-specific rule: allow only the reviewed `buzz-tools` server command/args/optionality with provider/env/headers/url absent, no root server, the two exact public Finch Git URLs with empty userinfo/query/fragment, bounded inert lock paths, and closed empty/absent auxiliary fields. The rule's `file|store|git` origin set does not override the production deploy path's separate `file`/`npm` rejection. Bind the producer's own rule pass/config digest in output v2 and never echo nested values. Derive Worker-owned values; validate/bind I268-owned phase/publication inputs without creating them. Persist the one closed v2 result through exclusive mode-`0600` same-directory temp creation, complete write, file sync, close, atomic hard-link no-replace, exact-inode verification, temp unlink, and directory fsync. Map `EEXIST` to `WORKER_RUNTIME_ADMISSION_ADAPTER_OUTPUT_EXISTS`, preserve competing bytes exactly, and clean only the owned temp. Map other persistence failures to `WORKER_RUNTIME_ADMISSION_ADAPTER_PERSIST_FAILED`; remove any linked target during failure cleanup only when a fresh identity check proves it is the owned inode. Provide no rename/copy/overwrite fallback. Add the package command without registering a deploy/network CLI control plane.
...
- The value-free Worker v2 adapter matches the exact reviewed I268 schema, confines both CLI paths, persists only through a mode-`0600` same-directory temp plus close/sync/hard-link no-replace/temp unlink/directory fsync, preserves a racing destination byte-for-byte, uses inode-scoped cleanup, emits no success stdout, and cannot become a control-plane/fallback path.
...
Stop, append a coordination event, and add a Notion checkpoint if G1/G2 is absent or changed; the adapter task lacks exact I268 G2 PASS; the runtime/filesystem cannot enforce same-filesystem hard-link no-replace plus directory fsync without fallback; origin changes frozen seams; lock preservation needs a broader break; validation cannot precede effects; caller data becomes derived authority; v1 would widen transport/auth/probes; a test needs network/credentials/publication/migration/deployment/mode/lease/candidate/traffic; or I267/I268 ownership would be crossed.
