...
This task is held until replacement I265 G2 passes. Exact I268 ref `75970eb0` has already received ordered I268 G1/G2 PASS for the original handshake, but the proposed identity-bearing failure payload is a process-contract amendment and therefore reopens that dependency. Before RED, obtain explicit immutable I268 plus architecture-coordinator acceptance of its exact stderr parser, key order, code/state mapping, exit status, and byte cap; then re-read the tracker/log, reverify all exact digests and accepted `cl.i268.finch-derivation-input.v1` / `cl.i268.finch-derivation-output.v2` schemas, and append the dependency transition to Notion/the shared log. No I265 proposal or silence authorizes the parser amendment.
...
20. inject failure of the no-mutation preflight directory sync and require `WORKER_RUNTIME_ADMISSION_OUTPUT_PERSISTENCE_UNSUPPORTED` before any temp or final entry exists; injected parse/identity/derivation/serialization/open/short-write/file-sync/close and conclusively uncommitted link failures otherwise emit only a stable sanitized stderr code, remove only a temp name still proven to be the captured owned inode, directory-sync any successful pre-commit cleanup unlink, leave no owned final before commit, and never truncate/overwrite/remove a pre-existing path; a directory/temp/final identity mismatch deletes no foreign object, and unsupported descriptor-bound hard-link or inode-revalidation semantics conclusively discovered before link fail closed as persistence-unsupported without a path-only or replace-capable fallback; after an owned final is observed, every failure uses its exact identity-bearing post-link outcome and is never reclassified as unsupported;
21. after explicit I268/coordinator process-contract acceptance, every outcome where commit cannot be conclusively excluded and every non-clean owned-final outcome exits `1` with exactly one canonical UTF-8 stderr JSON line no larger than 512 bytes and no stdout; pin byte-exact literal vectors for all five allowed `cl.i265.worker-runtime-admission-persistence-outcome.v1` code/state pairs, exact top-level/nested key order, no whitespace plus one LF, `retry: "forbidden"`, and the separate persistence consumer's bounded artifact identity; require both ambiguous-reconciliation observation cases to use that envelope and treat any identity not independently recomputable from safely observed bytes as recovery-only/non-qualifying; reject exit-status drift, extra/missing/reordered keys, oversize/noncanonical diagnostics, raw paths/output/commands/environment/credentials/secrets/exception text, self-identity inside output v2, and artifact identity on conclusive pre-commit or reconciled foreign-destination failure; those identity-free failures exit `1` with only stable code plus LF; and
