...
13. success serializes and size-checks complete canonical bytes in memory, exclusively creates one unpredictable same-directory temporary regular file at mode `0600` where supported, captures its identity, writes all bytes, syncs and closes it, then commits with same-filesystem `link(temporaryPath, finalPath)` as the atomic no-replace operation; ordinary rename, copy, direct-final write, and any replace-capable fallback are forbidden; success emits no stdout and leaves no temporary residue;
14. inject a competing destination after admission and the early absence precheck but immediately before `link`; require `EEXIST` to map to a stable sanitized output-exists failure, preserve the competing destination byte-for-byte, clean only the temporary pathname still proven to be the captured owned inode, and perform no retry or replace-capable fallback;
15. injected parse/identity/derivation/serialization/open/short-write/sync/close/link failures emit only a stable sanitized stderr code, remove only a temp pathname still proven to be the captured owned inode, leave no final result or owned-temp residue before commit, and never truncate/overwrite/remove a pre-existing path; a temp-identity mismatch deletes nothing; a post-link temp-cleanup failure leaves the committed final untouched and reports a distinct sanitized committed-cleanup result that cannot be retried blindly;
16. candidate/receipt self-identity is absent while the externally computed serialized-artifact identity remains downstream; and
17. hostile nested manifest command/environment values and lock path/Git URL values cause no process, filesystem, environment, network, output, or diagnostic side effect before rejection or successful pure derivation; forged caller `pass`, changed rule/config digest, URL userinfo/query/fragment, authorization/cookie/API-key header, unexpected env/header/provider, and missing/changed byte coverage fail without output or raw diagnostics.

### GREEN

Implement the minimum offline adapter around `cli/core/runtime-admission-manifest.ts`. Capture the process-start working-directory realpath as the sole confinement root; accept only the two reviewed CLI path operands under it; reject path escape, symlink, alias, existing-output, and non-regular-input cases; and create no directory. Accept only the reviewed bounded passive derivation-input file, reverify its closed byte identities, and perform no ambient lookup or execution. Treat every nested manifest/lock path, command, and URL as inert bytes. Independently run the exact frozen target-specific rule: allow only the reviewed `buzz-tools` server command/args/optionality with provider/env/headers/url absent, no root server, the two exact public Finch Git URLs with empty userinfo/query/fragment, bounded inert lock paths, and closed empty/absent auxiliary fields. The rule's `file|store|git` origin set does not override the production deploy path's separate `file`/`npm` rejection. Bind the producer's own rule pass/config digest in output v2 and never echo nested values. Derive Worker-owned values; validate/bind I268-owned phase/publication inputs without creating them. Persist the one closed v2 result through exclusive mode-`0600` same-directory temp creation, complete write, file sync, close, and an atomic no-replace hard-link commit. Treat the early destination check only as a diagnostic. On `EEXIST`, preserve the competing final byte-for-byte, remove only the temp still proven to be the captured owned inode, and fail with the stable sanitized output-exists code. Never fall back to ordinary rename, copy, direct-final write, or another replace-capable operation. Add the package command without registering a deploy/network CLI control plane.
...
- The value-free Worker v2 adapter matches the exact reviewed I268 schema, confines both CLI paths, persists only through a mode-`0600` same-directory temp plus close/sync/atomic no-replace hard-link commit, preserves a destination introduced after precheck byte-for-byte, cleans only its owned temp inode, emits no success stdout, and cannot become a control-plane/fallback path.
