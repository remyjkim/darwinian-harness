...
The two CLI operands `--input` and `--output` are the only adapter-control paths and are not fields in input v1. The process-start working directory is the explicit, pre-created, task-owned confinement root. Each operand must be a relative normalized path that resolves beneath that root; absolute paths, empty/dot paths, `..` traversal, NUL bytes, symlink components/targets, paths outside the root, non-regular input, a pre-existing final output, identical input/output identities, and missing output parents fail before derivation or output creation. The early destination-absence check is only a fast diagnostic; it is never commit authority. After canonical realpath admission, the adapter opens the already-existing output parent directory once and freezes its device/inode identity from the handle. Before temporary-file creation it must prove the canonical pathname still resolves to that admitted directory; parent substitution at this pre-open boundary fails without opening or writing outside the admitted inode. Temporary creation, temp/final identity checks, no-replace publication, and cleanup operate relative to that frozen directory handle through `openat`/`fstatat`/`linkat`/`unlinkat` semantics or an exact descriptor-bound equivalent; they never regain authority from the mutable canonical pathname. The adapter creates no directory and opens only the admitted regular input, the identity-bound output-parent handle, and its one owned output temporary/final entry. The runtime must support those descriptor-bound operations and directory sync or fail closed before publication.

After all validation and pure derivation succeed, the adapter serializes and size-checks the complete canonical output in memory. It then creates exactly one unpredictable same-directory temporary regular file relative to the frozen parent handle, with exclusive creation and mode `0600` where the platform supports POSIX modes, captures that file's identity, writes all bytes, syncs the file, and closes it. Immediately before publication it revalidates both invariants: the canonical parent pathname still has the frozen admitted device/inode, and descriptor-relative lookup proves the temporary name is the captured owned regular inode inside that directory. Parent or temp substitution at this pre-link boundary fails closed and cleanup remains limited to the proven-owned temp. Final publication uses descriptor-bound `linkat(parentHandle, temporaryName, parentHandle, finalName)` or an exact same-directory equivalent, not ordinary path-only `link`, rename, copy, or direct final-path write. The successful hard link is the atomic no-replace namespace commit point: it gives the final name to the already closed and synced inode only while that name is absent. `EEXIST` means a competing destination won the race and must map to a stable sanitized output-exists failure; the adapter does not read, truncate, rename over, unlink, chmod, or otherwise change that destination. There is no replace-capable or path-only fallback on platforms or filesystems where descriptor-bound hard links, inode revalidation, and opened-directory sync cannot establish this contract.

Immediately after successful `link`, and again before returning any identity or clean success, the adapter proves that the canonical parent pathname still has the frozen admitted device/inode and that the final target is a regular file with the captured temporary inode identity. A parent or final-target mismatch fails closed, qualifies no artifact, removes or rewrites no final target, forbids blind retry, and cleans only a temporary path still independently proven to be the owned inode. Path-only confinement or a once-only admission check is insufficient.

The hard link alone is not crash-durable publication. After the immediate post-link identity checks succeed, the adapter syncs the opened parent-directory handle before unlinking the temporary name. Only a successful first directory sync makes the validated final name durably committed. A first directory-sync failure returns sanitized `WORKER_RUNTIME_ADMISSION_OUTPUT_COMMIT_INDETERMINATE`, leaves both final and temporary names untouched for recovery, never claims success, and forbids blind retry because a later crash may preserve either namespace state. Once the first directory sync succeeds, the committed final is never removed, replaced, or rewritten.

Before unlinking a temporary name on either success or failure, descriptor-relative identity lookup must prove it still resolves to the adapter-owned captured inode; an identity mismatch leaves the name untouched and fails closed rather than deleting another writer's object. Cleanup unlinks only that proven name relative to the frozen parent handle, never through a substituted pathname. After durable final commit, failure to unlink that owned name returns sanitized `WORKER_RUNTIME_ADMISSION_OUTPUT_COMMITTED_TEMP_CLEANUP_FAILED`, preserves the committed final and temporary name, never claims clean success, and forbids blind retry. After a successful unlink, the adapter syncs the same opened parent-directory handle a second time. Failure of that second sync returns sanitized `WORKER_RUNTIME_ADMISSION_OUTPUT_COMMITTED_CLEANUP_DURABILITY_INDETERMINATE`: the final remains durably committed and untouched, the temporary name is absent in the live namespace, but its removal is not crash-durable. Only descriptor-bound link plus first directory sync plus owned-temp unlink plus second directory sync is clean success; it emits no stdout and leaves no temporary residue.

Before namespace commit, parse, identity, derivation, serialization, open, short-write, file-sync, close, or link failure removes only the still-owned temporary inode and syncs the parent directory after a successful cleanup unlink. `EEXIST` follows that same identity-safe, directory-synced temp cleanup while preserving the competing final byte-for-byte. An unsupported hard link or directory open/sync maps to sanitized `WORKER_RUNTIME_ADMISSION_OUTPUT_PERSISTENCE_UNSUPPORTED`; no rename/copy fallback is permitted. A pre-commit cleanup-sync failure is also indeterminate and never clean success. None of these outcomes removes or rewrites a final path.
...
| Independent I268 comparison handshake | packaged Worker v2 adapter exact-file/command tests, frozen parent dev/ino with pre-open, pre-link, post-link, and pre-success revalidation; mode-0600 same-directory temp, file sync, atomic no-replace hard-link commit, first parent-directory sync, identity-safe unlink, second parent-directory sync, adversarial destination/parent races, explicit indeterminate/committed-cleanup outcomes, no success stdout, plus dual G2 receipt |
