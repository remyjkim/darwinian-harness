...
The identity-bearing failure payload below is a proposed amendment to the accepted I268-owned process handshake, not an accepted interface. I268 retains strict process invocation and parser ownership. Task 8 RED/source cannot begin until I268 explicitly acknowledges the exact payload/exit contract at an immutable reviewed ref and the architecture coordinator records the dependency acceptance; silence, this I265 proposal, or compatible-looking implementation is insufficient.

Once accepted, every non-clean outcome for which namespace commit cannot be conclusively excluded, plus every non-clean outcome after an owned final is observed, exits `1`, emits exactly one canonical UTF-8 JSON line no larger than 512 bytes on stderr, and emits no stdout. The closed payload schema is `cl.i265.worker-runtime-admission-persistence-outcome.v1`. Serialization is byte-exact `JSON.stringify` form with no optional whitespace, one trailing LF, top-level key order `schema`, `code`, `commitState`, `retry`, `artifactIdentity`, and nested identity key order `schema`, `phase`, `byteLength`, `sha256`. `retry` is exactly `"forbidden"`; `artifactIdentity` is the separate persistence consumer's I268 tuple. Missing, extra, reordered, malformed, oversize, or noncanonical bytes fail I268 parsing and grant no retry or qualification. The payload contains no path, raw output, command, environment, credential, secret, exception text, or mutable target.
...
Conclusive pre-commit failures and reconciled foreign-destination failures instead exit `1` with only their stable sanitized code plus LF on stderr, no stdout, and no artifact identity. Exact RED vectors use the fixed non-qualifying identity to pin all five JSON code/state strings byte-for-byte, including the `512`-byte ceiling; those vectors are process-contract fixtures, never Finch authority.
...
Event `i265-owner-20260809T235736Z-015` records I265's fresh exact-digest acceptance and supersedes only the verdict of the premature event, not its audit history. Any later schema, bound, file, command, runtime, version, ownership, lookup, or rule change invalidates acceptance. The proposed identity-bearing failure payload is such a process-contract change. Adapter RED/source requires I265 G2 PASS, the corrected immutable I268 schema ref's ordered G1 then G2 PASS, and a later explicit I268/coordinator acceptance of this exact amendment. The synthetic vector remains non-authoritative. I265's core declaration/producer/materializer tasks require I265 G2 but do not wait on unrelated I268 publication authority.
