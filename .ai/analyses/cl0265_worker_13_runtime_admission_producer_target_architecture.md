...
The two CLI operands `--input` and `--output` are the only adapter-control paths and are not fields in input v1. The process-start working directory is the explicit, pre-created, task-owned confinement root. Each operand must be a relative normalized path that resolves beneath that root; absolute paths, empty/dot paths, `..` traversal, NUL bytes, symlink components/targets, paths outside the root, non-regular input, a pre-existing final output, identical input/output identities, and missing output parents fail before derivation or output creation. The adapter creates no directory and opens no file except the admitted regular input and its one owned output temporary/final path.

After all validation and pure derivation succeed, the adapter serializes and size-checks the complete canonical output in memory. It then creates exactly one unpredictable same-directory temporary file with exclusive creation and mode `0600` where the platform supports POSIX modes, records its device/inode identity, verifies that it is the owned regular file, writes all bytes, syncs the file, and closes it.

The final namespace commit is an atomic same-filesystem hard link from that closed temporary inode to the absent target. Ordinary rename, copy, truncate, overwrite, or any compatibility fallback is forbidden. The hard-link primitive must fail with destination-exists semantics without replacing the target; if the running platform/filesystem cannot enforce that primitive, the adapter fails closed. A destination created after the last absence observation therefore causes `EEXIST`, maps to stable code `WORKER_RUNTIME_ADMISSION_ADAPTER_OUTPUT_EXISTS`, preserves the competing destination byte-for-byte, returns no result identity, and removes only the exact owned temporary inode.

After a successful link, the adapter verifies that the final path is the same owned regular inode, unlinks the temporary name, and syncs the containing directory before returning success. A link, post-link identity, temp-unlink, or directory-sync failure maps to `WORKER_RUNTIME_ADMISSION_ADAPTER_PERSIST_FAILED`; cleanup may remove the final or temporary name only after a fresh device/inode comparison proves that name still identifies the owned inode. Success emits no stdout. Parse, identity, derivation, serialization, open, short-write, file-sync, close, hard-link, unlink, and directory-sync failures emit only a stable sanitized code on stderr and no result identity. No failure path truncates, overwrites, unlinks, or otherwise mutates a pre-existing or competing destination.
...
| Independent I268 comparison handshake | packaged Worker v2 adapter exact-file/command tests, confined paths, mode-0600 same-directory temp, close/sync, hard-link no-replace, competing-byte preservation, inode-scoped cleanup, directory fsync, no success stdout, plus dual G2 receipt |
