...
After all validation and pure derivation succeed, the adapter serializes and size-checks the complete canonical output in memory. It then creates exactly one unpredictable same-directory temporary file with exclusive creation and mode `0600` where the platform supports POSIX modes, verifies that it is the owned regular file, writes all bytes, syncs the file, and closes it before an atomic same-directory rename to the still-absent final output. The rename is the commit point and no fallible persistence step follows it. Success emits no stdout. Parse, identity, derivation, serialization, open, short-write, sync, close, or rename failure emits only a stable sanitized code on stderr, removes only the adapter-owned temporary file, and leaves neither a final result nor temporary residue; it never truncates, overwrites, or removes a pre-existing path.
...
| Independent I268 comparison handshake | packaged Worker v2 adapter exact-file/command tests, confined paths, mode-0600 same-directory temp, close/sync/atomic rename, failure cleanup, no success stdout, plus dual G2 receipt |
