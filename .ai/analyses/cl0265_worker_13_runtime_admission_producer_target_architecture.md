...
The two CLI operands `--input` and `--output` are the only adapter-control paths and are not fields in input v1. The process-start working directory is the explicit, pre-created, task-owned confinement root. Each operand must be a relative normalized path that resolves beneath that root; absolute paths, empty/dot paths, `..` traversal, NUL bytes, symlink components/targets, paths outside the root, non-regular input, a pre-existing final output, identical input/output identities, and missing output parents fail before derivation or output creation. The early destination-absence check is only a fast diagnostic; it is never commit authority. The adapter creates no directory and opens no file except the admitted regular input and its one owned output temporary/final path.

After all validation and pure derivation succeed, the adapter serializes and size-checks the complete canonical output in memory. It then creates exactly one unpredictable same-directory temporary regular file with exclusive creation and mode `0600` where the platform supports POSIX modes, captures that file's identity, writes all bytes, syncs the file, and closes it. Final publication uses the same-filesystem `link(temporaryPath, finalPath)` primitive, not ordinary rename, copy, or direct final-path write. The successful hard link is the atomic no-replace commit point: it gives the final name to the already closed and synced inode only while that name is absent. `EEXIST` means a competing destination won the race and must map to a stable sanitized output-exists failure; the adapter does not read, truncate, rename over, unlink, chmod, or otherwise change that destination. There is no replace-capable fallback on platforms or filesystems where this contract cannot be established.

Before unlinking a temporary pathname on either success or failure, cleanup must prove it still resolves to the adapter-owned captured inode; an identity mismatch leaves the pathname untouched and fails closed rather than deleting another writer's object. After a successful hard-link commit, removal of the owned temporary name is cleanup only and cannot alter the final inode. If that cleanup cannot complete, the adapter reports a distinct sanitized post-commit cleanup failure that states only that the final output was committed and must not be retried blindly; it never removes or rewrites the committed final. Before commit, parse, identity, derivation, serialization, open, short-write, sync, close, or link failure removes only the still-owned temporary inode and leaves no final result. Success emits no stdout and leaves no temporary residue.
...
| Independent I268 comparison handshake | packaged Worker v2 adapter exact-file/command tests, confined paths, mode-0600 same-directory temp, close/sync/atomic no-replace hard-link commit, adversarial destination race, identity-safe cleanup, no success stdout, plus dual G2 receipt |
