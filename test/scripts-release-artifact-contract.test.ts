...
describe("packed artifact measurement", () => {
  test("requalifies a downloaded tar from measured receipt fields without trusting original pack output", async () => {
    ...
    const artifactPath = join(root, "darwinian-1.4.0.tgz");
  ...
  test("parses one actual pack result and binds byte identities to checkout and build identity", async () => {
    ...
    await writeFile(join(root, "darwinian-1.4.0.tgz"), bytes);
  ...
  test.each([
    ["path traversal", { filename: "../darwinian-1.4.0.tgz" }],
    ["absolute path", { filename: "/tmp/darwinian-1.4.0.tgz" }],
  ...
  test("rejects mismatched build and checkout tuples", async () => {
    for (const scenario of [
      ...
      { identity: { version: "1.4.0", sourceCommit: "b".repeat(40) }, checkoutCommit: SOURCE_COMMIT },
      { identity: { version: "1.4.0", sourceCommit: SOURCE_COMMIT }, checkoutCommit: "A".repeat(40) },
    ]) {
      const root = await tempRoot();
      const bytes = Buffer.from("qualified tar bytes");
      await writeFile(join(root, "darwinian-1.4.0.tgz"), bytes);
...
describe("installed artifact smokes", () => {
  test("installs into an isolated prefix/cache and invokes only the installed bin for every safe smoke", async () => {
    const root = await tempRoot();
    const artifact = join(root, "darwinian-1.4.0.tgz");
    ...
    const runner: ReleaseCommandRunner = async (command, options) => {
      ...
      return { exitCode: 0, stdout: command.includes("--version") ? "1.4.0\n" : "Usage\n", stderr: "" };
    };
    ...
    expect(result).toEqual({ version: "1.4.0", passed: SAFE_INSTALLED_SMOKES.map((smoke) => smoke.join(" ")) });
  ...
  test("fails when version output differs or a help smoke mutates quarantined auth/project state", async () => {
    ...
    const runner: ReleaseCommandRunner = async (command, options) => {
      ...
      return { exitCode: 0, stdout: smokeCount === 1 ? "1.4.0\n" : "Usage\n", stderr: "" };
    };
    await expect(runInstalledArtifactSmokes({ artifactPath: artifact, expectedVersion: "1.4.0", workspaceRoot }, {
      run: runner,
      resolveInstalledBin: async (prefix) => join(await realpath(prefix), "lib", "node_modules", "darwinian", "cli", "index.ts"),
...
describe("published registry byte identity", () => {
  ...
  test("fails on mismatches, malformed metadata, and absent gitHead when recovery requires it", () => {
    for (const metadata of [
      ...
      { version: "1.4.0", gitHead: "b".repeat(40), dist: { shasum: expected.sha1, integrity: expected.integrity } },
      { version: "1.4.0", gitHead: SOURCE_COMMIT, dist: { shasum: "d".repeat(40), integrity: expected.integrity } },
      { version: "1.4.0", gitHead: SOURCE_COMMIT, dist: { shasum: expected.sha1, integrity: "bad" } },
      { version: "1.4.0", gitHead: SOURCE_COMMIT, dist: { shasum: expected.sha1, integrity: expected.integrity }, extra: true },
    ]) {
