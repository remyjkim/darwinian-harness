...
describe("Worker contract release gate", () => {
  ...
  test("requires exact 1.4.2 package identity derived by the runtime while preserving the 1.1.0 hard-cut floor", () => {
    const result = verifyWorkerContract(repoRoot, {
    ...
    expect(result.ok).toBe(false);
    expect(result.details).toContain("package version must be 1.4.2");
