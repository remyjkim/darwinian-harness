...
async function scaffoldCleanRepo(version = "1.4.0"): Promise<string> {
...
describe("packaged Worker build identity", () => {
  test("accepts only the exact schema with package-version parity and a lowercase full Git SHA", () => {
    ...
    expect(parsePackagedBuildIdentity(input, "1.4.0")).toEqual({
      kind: "packaged",
      ...input,
    ...
    for (const invalid of [
      ...
      expect(() => parsePackagedBuildIdentity(invalid, "1.4.0")).toThrow(BuildIdentityError);
    }
    expect(() => parsePackagedBuildIdentity(input, "v1.4.0")).toThrow(BuildIdentityError);
  });
  ...
  test("fails closed rather than treating malformed or mismatched present members as development", async () => {
    ...
    for (const contents of [
      "not-json",
      JSON.stringify({ schema: "darwinian.worker.build-identity", schemaVersion: 1, version: "1.4.0" }),
      JSON.stringify({
        schema: "darwinian.worker.build-identity",
        schemaVersion: 1,
        version: "1.1.0",
...
describe("build identity generator", () => {
  ...
  test("checks cleanliness before deriving HEAD and accepts no caller-supplied version or commit", async () => {
    ...
    expect(generated.version).toBe("1.4.0");
