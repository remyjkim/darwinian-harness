...
## Releasing a new CLI version: Darwinian Worker 1.4.0

The CLI release is a two-event, exact-byte process. Manual dispatch can only
qualify current `main`; it cannot publish. A later exact annotated `v1.4.0` tag
...
`1.4.0`. A source/development identity is explicitly non-eligible. Source
...
Dispatch `CLI Release` from `main` with version `1.4.0` and `dry_run: true`.
The workflow rejects any other ref, a moved `origin/main`, a package-version
mismatch, an indeterminate registry result, or an existing `darwinian@1.4.0`.
It runs the full qualification gates, packs once, installs once, and uploads
exactly:

- `darwinian-1.4.0.tgz`; and
...
- GitHub environment `darwinian-npm-publish` matches the declared approval
  ...
  admits only tag `v1.4.0`. Under the current policy the tag is created by
...
Create and inspect the annotated `v1.4.0` tag locally before pushing it:

```bash
git tag -a v1.4.0 --file release-tag-message.txt
git cat-file -p refs/tags/v1.4.0
...
```bash
npm publish "./candidate/darwinian-1.4.0.tgz" --access public
```
...
```bash
npm view darwinian@1.4.0 version gitHead dist.shasum dist.integrity --json
```
...
fails, the ordinary workflow cannot be rerun because `1.4.0` now exists. Use
`.github/workflows/release-recovery.yml` only after a new policy-conformant approval.

Dispatch `CLI Release Recovery` with ref `v1.4.0`, the exact failed canonical
